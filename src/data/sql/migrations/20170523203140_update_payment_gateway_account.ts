    ...
    return promise.resolve()
        .then(() => {
            return knex.raw(`ALTER TABLE ${Schema.PAYMENT_GATEWAY_ACCOUNT_TABLE_SCHEMA.TABLE_NAME}
                        ALTER COLUMN ${Schema.PAYMENT_GATEWAY_ACCOUNT_TABLE_SCHEMA.FIELDS.MERCHANT_ID} DROP NOT NULL;`);
        });
};
...
export const down = (knex: Knex, promise: typeof Bluebird): Bluebird<any> => {
    return promise.resolve()
        .then(() => {
            return knex.raw(`ALTER TABLE ${Schema.PAYMENT_GATEWAY_ACCOUNT_TABLE_SCHEMA.TABLE_NAME}
                        ALTER COLUMN ${Schema.PAYMENT_GATEWAY_ACCOUNT_TABLE_SCHEMA.FIELDS.MERCHANT_ID} SET NOT NULL;`);
        });
};
