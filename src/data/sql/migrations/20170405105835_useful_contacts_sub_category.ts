...
let initCategory = (knex: Knex, promise: typeof Bluebird): Bluebird<any> => {
    let SCHEMA = Schema.USEFUL_CONTACT_SUB_CATEGORY_TABLE_SCHEMA;
    ...
    return promise.resolve()
        ...
        .then(() => {
            return knex.schema.createTable(SCHEMA.TABLE_NAME, (table) => {
                ...
                table.text(SCHEMA.FIELDS.CATEGORY_ID).notNullable().index()
...
export const down = (knex: Knex, promise: typeof Bluebird) => {
    return promise.all([
        knex.schema.raw(`DROP TABLE ${Schema.USEFUL_CONTACT_SUB_CATEGORY_TABLE_SCHEMA.TABLE_NAME} CASCADE`),
    ]);
};
